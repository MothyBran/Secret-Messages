...
module.exports = (dbQuery, upload) => {
    ...
    // 2c. GET POST DETAIL (Enterprise/Public view in Hub)
    router.get('/api/posts/:id', verifySession, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            if (isNaN(id) || id < 1) return res.status(400).json({ error: "Invalid ID" });

            // In Enterprise, verifySession ensures req.user is populated.
            // Check if admin to show drafts
            const showDrafts = (req.user.isAdmin === 1 || req.user.isAdmin === true);
