...
// ==================================================================
// ADMIN API ROUTES (Fehlte wahrscheinlich)
// ==================================================================
...
// ==================================================================
// 5. PAYMENT & RENEWAL ROUTES (FIXED)
// ==================================================================

// A) CHECKOUT SESSION ERSTELLEN (Für Shop & Verlängerung)
app.post('/api/create-checkout-session', async (req, res) => {
    try {
        ...
        if (is_renewal) {
            ...
            // Standard Shop Kauf (Neue Lizenz)
...
// B) ORDER STATUS / FULFILLMENT (Verlängert Key oder erstellt neuen)
app.get('/api/order-status', async (req, res) => {
    ...
    try {
        ...
        if (session.payment_status === 'paid') {
            ...
            // Prüfen ob Zahlung schon verarbeitet wurde
            ...
            if (checkPay.rows.length === 0) {
                ...
                if (meta.type === 'renewal' && meta.license_key_id) {
                    ...
                    // SQL Update für PostgreSQL oder SQLite
                    ...
                    // (Dies ist die Logik, die in deiner alten server.js für neue Keys war)
                    const productMap = {
                        '1m': '1m', '3m': '3m', '12m': '12m', 'unlimited': 'unlimited',
                        'bundle_1m_2': '1m', 'bundle_3m_2': '3m', 'bundle_3m_5': '3m', 'bundle_1y_10': '12m'
                    };
                    
                    const countMap = {
                        'bundle_1m_2': 2, 'bundle_3m_2': 2, 'bundle_3m_5': 5, 'bundle_1y_10': 10
                    };

                    const finalProduct = productMap[meta.product_type] || '1m';
                    const count = countMap[meta.product_type] || 1;
                    const newKeys = [];

                    for(let i=0; i<count; i++) {
                         // Key Generierung (identisch zu deiner generate-keys Logik)
                         const keyRaw = crypto.randomBytes(16).toString('hex').toUpperCase().match(/.{4}/g).join('-');
                         const keyHash = await bcrypt.hash(keyRaw, 10);
                         
                         // Berechnung Ablaufdatum (Standard: ab Aktivierung, hier NULL lassen)
                         // oder direkt setzen wenn gewünscht. Wir lassen es auf Aktivierung warten.
