// ==================================================================
// SERVER.JS - FINAL VERSION (SQLITE & POSTGRES COMPATIBLE)
// ==================================================================
        ...
        dbQuery = (text, params = []) => {
            return new Promise((resolve, reject) => {
                ...
                if (text.trim().toUpperCase().startsWith('SELECT')) {
                    db.all(sql, params, (err, rows) => { if (err) reject(err); else resolve({ rows: rows, rowCount: rows.length }); });
                } else {
                    db.run(sql, params, function(err) { if (err) reject(err); else resolve({ rows: [], rowCount: this.changes, lastID: this.lastID }); });
                }
            });
        };
        createTables();
...
const createTables = async () => {
    try {
        await dbQuery(`CREATE TABLE IF NOT EXISTS users (id ${isPostgreSQL ? 'SERIAL PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT'}, username VARCHAR(50) UNIQUE, access_code_hash TEXT, license_key_id INTEGER, allowed_device_id TEXT, registered_at ${isPostgreSQL ? 'TIMESTAMP DEFAULT CURRENT_TIMESTAMP' : 'DATETIME DEFAULT CURRENT_TIMESTAMP'}, last_login ${isPostgreSQL ? 'TIMESTAMP' : 'DATETIME'}, is_blocked ${isPostgreSQL ? 'BOOLEAN DEFAULT FALSE' : 'INTEGER DEFAULT 0'}, is_online ${isPostgreSQL ? 'BOOLEAN DEFAULT FALSE' : 'INTEGER DEFAULT 0'})`);
        await dbQuery(`CREATE TABLE IF NOT EXISTS license_keys (id ${isPostgreSQL ? 'SERIAL PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT'}, key_code VARCHAR(17) UNIQUE NOT NULL, key_hash TEXT NOT NULL, product_code VARCHAR(10), created_at ${isPostgreSQL ? 'TIMESTAMP DEFAULT CURRENT_TIMESTAMP' : 'DATETIME DEFAULT CURRENT_TIMESTAMP'}, activated_at ${isPostgreSQL ? 'TIMESTAMP' : 'DATETIME'}, expires_at ${isPostgreSQL ? 'TIMESTAMP' : 'DATETIME'}, is_active ${isPostgreSQL ? 'BOOLEAN DEFAULT FALSE' : 'INTEGER DEFAULT 0'}, username VARCHAR(50), activated_ip VARCHAR(50))`);
        await dbQuery(`CREATE TABLE IF NOT EXISTS payments (id ${isPostgreSQL ? 'SERIAL PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT'}, payment_id VARCHAR(100), amount INTEGER, currency VARCHAR(10), status VARCHAR(20), payment_method VARCHAR(50), completed_at ${isPostgreSQL ? 'TIMESTAMP' : 'DATETIME'}, metadata TEXT)`);
...
async function authenticateUser(req, res, next) {
    ...
    jwt.verify(token, JWT_SECRET, async (err, user) => {
        ...
        try {
            ...
            if (!dbUser || dbUser.is_blocked) return res.status(403).json({ error: "Konto gesperrt." });
            req.user = user;
            next();
        } catch (dbError) { return res.status(500).json({ error: 'Auth Fehler.' }); }
    });
}
...
app.post('/api/auth/login', rateLimiter, async (req, res) => {
    try {
        ...
        if (user.allowed_device_id && user.allowed_device_id !== deviceId) return res.status(403).json({ success: false, error: "Gerät nicht autorisiert." });
        if (!user.allowed_device_id) await dbQuery("UPDATE users SET allowed_device_id = $1 WHERE id = $2", [deviceId, user.id]);
...
app.post('/api/auth/activate', async (req, res) => {
    ...
    try {
        ...
        if (!key || key.activated_at) return res.status(403).json({ error: 'Key ungültig' });
        ...
        await dbQuery('INSERT INTO users (username, access_code_hash, license_key_id, allowed_device_id, registered_at) VALUES ($1, $2, $3, $4, $5)', [username, hash, key.id, deviceId, new Date().toISOString()]);
        await dbQuery('UPDATE license_keys SET is_active = $1, activated_at = $2, username = $3 WHERE id = $4', [(isPostgreSQL ? true : 1), new Date().toISOString(), username, key.id]);
        res.json({ success: true });
...
app.post('/api/auth/validate', async (req, res) => {
    ...
    try {
        ...
        if (userRes.rows.length > 0) res.json({ valid: true, username: userRes.rows[0].username, expiresAt: userRes.rows[0].expires_at });
...
const requireAdmin = (req, res, next) => {
    ...
    const pass = password || req.headers['x-admin-password'] || req.body.password;
...
app.get('/api/admin/stats', requireAdmin, async (req, res) => {
    try {
        ...
        res.json({ success: true, stats: { users_active: activeUsers.rows[0].c, users_blocked: blockedUsers.rows[0].c, keys_active: activeKeys.rows[0].c, keys_expired: expiredKeys.rows[0].c, purchases_count: totalPurchases.rows[0].c, revenue_total: (totalRevenue.rows[0].s || 0) } });
...
// KEY UPDATE (FIXED: Safe SQL for Boolean)
    ...
    try {
        ...
        if (user_id) {
            ...
            if (userCheck.rows.length > 0) {
                ...
                // Hier wurde der Bug behoben (true/1):
...
// GENERATE KEYS (FIXED: Safe SQL for Boolean)
app.post('/api/admin/generate-keys', requireAdmin, async (req, res) => {
    try {
        ...
        for(let i=0; i < amount; i++) {
            ...
            // Hier wurde der Bug behoben (true/1):
            await dbQuery(`INSERT INTO license_keys (key_code, key_hash, product_code, is_active) VALUES ($1, $2, $3, ${isPostgreSQL ? 'true' : '1'})`, [keyRaw, keyHash, finalProduct]);
            newKeys.push(keyRaw);
        }
        res.json({ success: true, keys: newKeys });
    } catch (e) { console.error("Gen Error:", e); res.status(500).json({ error: e.message }); }
});
...
app.get('/api/admin/users', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await dbQuery(sql);
        const users = result.rows.map(r => ({ ...r, is_blocked: isPostgreSQL ? r.is_blocked : (r.is_blocked === 1) }));
        res.json(users);
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/admin/purchases', requireAdmin, async (req, res) => {
    try {
        ...
        const purchases = result.rows.map(r => {
            ...
            return { id: r.payment_id, email: meta.email || meta.customer_email || '?', product: meta.product_type || '?', amount: r.amount, currency: r.currency, date: r.completed_at, status: r.status };
        });
        res.json(purchases);
    } catch (e) { res.json([]); }
});
...
app.post('/api/admin/block-user/:id', requireAdmin, async (req, res) => {
    try {
        ...
        await dbQuery("DELETE FROM user_sessions WHERE user_id = $1", [req.params.id]); // Optional table, but good practice
        res.json({ success: true });
