...
app.post('/api/admin/users', async (req, res) => {
  ...
  try {
    ...
    const sql = isPostgreSQL
      ? `SELECT  u.id,  u.username,  u.is_blocked,  u.registered_at,  u.last_login,  lk.key_code,  lk.product_code,  lk.activated_at,  lk.expires_at,  MAX(us.last_activity) AS last_used_at FROM users u LEFT JOIN license_keys lk ON lk.id = u.license_key_id LEFT JOIN user_sessions us ON us.user_id = u.id GROUP BY u.id, u.username, u.is_blocked, u.registered_at, u.last_login,     lk.key_code, lk.product_code, lk.activated_at, lk.expires_at ORDER BY COALESCE(MAX(us.last_activity), lk.activated_at, u.registered_at) DESC LIMIT $1 OFFSET $2`
      : `SELECT  u.id,  u.username,  u.is_blocked,  u.registered_at,  u.last_login,  lk.key_code,  lk.product_code,  lk.activated_at,  lk.expires_at,  MAX(us.last_activity) AS last_used_at FROM users u LEFT JOIN license_keys lk ON lk.id = u.license_key_id LEFT JOIN user_sessions us ON us.user_id = u.id GROUP BY u.id, u.username, u.is_blocked, u.registered_at, u.last_login,     lk.key_code, lk.product_code, lk.activated_at, lk.expires_at ORDER BY COALESCE(MAX(us.last_activity), lk.activated_at, u.registered_at) DESC LIMIT $1 OFFSET $2`
